...
The release-specific support status for installing OKF Harness into an agent client. Levels are `native-supported`, `manual-supported`, `install-later`, and `unsupported`; default setup selection is a separate behavior so a native-supported integration can still require explicit opt-in.
_Avoid_: quality score, model support, runtime compatibility
...
The universal setup step where OKF Harness automatically detects supported agent clients on the machine, then asks the person which detected or supported agent integrations to install. Detection is automatic; choosing where OKF Harness is installed remains explicit.
_Avoid_: silent auto-install, manual agent discovery, adapter prompt

**Undetected agent option**:
A supported agent client shown by universal setup even when it is not present on the machine. Selecting it should return the OKF Harness install command to run after that agent client is installed, not explain how to install the agent client or create speculative files for software the person has not installed.
...
A detected supported agent client that OKF Harness cannot safely install through a stable native channel in the current release. Universal setup may show it as detected, but it should not preselect it or write integration files without a clear manual instruction path.
_Avoid_: broken native install, silent fallback, speculative adapter
