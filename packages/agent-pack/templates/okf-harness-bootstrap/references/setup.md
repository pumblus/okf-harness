...
Report the workspace path and the `data.refresh` message from the CLI. If `data.refresh.commands` exists, show the two command lines exactly; otherwise repeat the natural-language refresh message before telling the user to use `{{workspaceInvocation}}` in the fresh {{sessionName}}.
