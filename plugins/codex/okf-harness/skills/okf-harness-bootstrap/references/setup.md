...
Report the workspace path, whether Git was initialized, and the `data.refresh` message from the CLI. If `data.refresh.commands` exists, show the two command lines exactly; otherwise repeat the natural-language refresh message before telling the user to use `$okf-harness` in the fresh Codex thread.
