...
`doctor` does not write files. For a pin-less workspace, the `workspace-runtime-pin` check reports `details.adoptCommand` as an exact executable-and-arguments object that runs `@okf-harness/cli@0.8.0` through `npx`; it does not require a global `okfh`.
