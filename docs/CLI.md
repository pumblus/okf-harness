...
Each `seals` entry carries the condition `code`, source anchor (`sourceId` and `sourcePath` when registered), affected concept IDs under `sealed`, and a factual `basis`. It contains no repair advice. Human output shows the same seal fields. `limits` now reports only mechanical boundaries such as no matches or truncation; these physical source-damage conditions are carried by `seals`, not `WORKSPACE_RISK`.
