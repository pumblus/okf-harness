...
- GUI、云端同步、用户账户、向量搜索、RAG、自动网页抓取、Obsidian 运行时代码和跨平台运行时支持，会留在需求分层里，直到它们能保留本地化、可检查的工作流
...
约束：跨平台支持应作为明确的加固工作推进，而不是通过零散改动让当前 macOS 优先路径变复杂。
