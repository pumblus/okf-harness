...
它与完整桌面 LLM Wiki 应用的核心区别是有意为之：OKF Harness 不试图成为主要的知识库应用。你继续在受支持智能体客户端中工作；harness 在普通 markdown 文件之上提供透明的本地 workspace、确定性 CLI、受控证据、受控读取、lint 和图谱报告。
...
- 回答基于已整理 `wiki/` 内容中的受控证据，而非跨原始资料的全文发现
- 没有 `okfh query` 或 LLM 回答命令；agent 先准备 Evidence Brief，再根据返回的证据和限制回答
- 搜索结果返回候选概念文档，不是答案证据，仍作为更底层的调试工具
- 读取默认受控，并为 evidence 续读提供显式选项
...
目标：支持更多 agent 客户端，并加深现有集成，同时不削弱默认的本地 shell 模型。
