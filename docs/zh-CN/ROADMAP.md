...
目标：不增加新产品面，只让 v0.3 的承诺真正成立。

补丁范围：

- 正确接受 bundle 根 `index.md` 上合法的 OKF bundle metadata。
- 让生成的 Claude Code 和 Codex skill 保持严格、版本可信，并从 Markdown 模板维护。
- 将 `doctor` 拆成普通运行环境检查和 `doctor --dev` 仓库开发环境检查。
- 备份旧 workflow-specific skill 目录，再把它们从可发现的 skill 路径中移除。
- 暴露安全策略缺口，但不声称已经执行自动 Git checkpoint。
- 用 npm tarball 验证 release 行为，而不只验证 monorepo 源码。
- 保留 `check` 作为常规验证工作流；`lint` 继续退休。

约束：

- 这个补丁不加入证据规划、GUI、新 adapter、云同步、向量搜索或自动 checkpoint。
...
发布方向：v0.3.1 信任修复补丁之后，v0.4 应聚焦受控证据规划，而不是继续调整 skill 架构。

候选范围：

- 一个未来命令或工作流，用于在 agent 回答前准备受控证据包
