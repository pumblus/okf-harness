...
感谢你花时间改进 OKF Harness。项目目前还很小，最好的贡献是范围窄、可测试、且与 v0.1 本地 agent 优先方向一致的改动。
...
直接提交将 v0.1 推向 MCP 优先、云端账户、后台爬取、自动修改原始资料或非 JSON agent 契约的 PR，将被关闭或引导调整方向。
...
- 保持 `@okf-harness/core` 独立于 CLI、agent-pack 和 MCP 包。
- 保持 `@okf-harness/cli` 作为 core 和 agent-pack 行为之间的桥梁。
- 保持 MCP 为可选项，不进入 v0.1 默认路径。
...
README 应面向用户、篇幅简短。命令参考放在 [docs/CLI.md](CLI.md)，用户工作流放在 [docs/WORKFLOWS.md](WORKFLOWS.md)，路线图想法放在 [docs/ROADMAP.md](ROADMAP.md)，实现细节放在 [docs/implementation.md](../implementation.md)。
